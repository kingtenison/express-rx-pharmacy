...
import { Heart, Zap, CreditCard, UserCheck, FlaskConical, MapPin } from "lucide-react";
...
export default function FeatureGrid() {
  ...
  return (
    <section ref={ref} className="relative py-12 md:py-20" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-8 md:mb-12">
          ...
          <h2 className="display-lg mb-4" style={{ color: "var(--text-1)" }}>The ExpressRX Difference</h2>
          <p className="section-subtitle mx-auto" style={{ color: "var(--text-2)" }}>We go above and beyond to provide exceptional care and service.</p>
        </motion.div>
              ...
              <motion.div key={i} variants={itemVariants}>
                <motion.div className="card-bento h-full" whileHover={{ y: -4 }} transition={{ duration: 0.3 }}>
                  <div className="relative z-10 p-4 md:p-6">
                    ...
                    <h3 className="text-base md:text-xl font-semibold mb-2" style={{ color: "var(--text-1)" }}>{feature.title}</h3>
                    <p className="text-xs md:text-sm leading-relaxed" style={{ color: "var(--text-2)" }}>{feature.description}</p>
                  </div>
                </motion.div>
              </motion.div>
            );
